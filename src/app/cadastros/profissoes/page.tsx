...
import TabelaProfissoes, { Profissao } from "@/components/tabelas/tabelaprofissoes";
...
export default function ProfissoesPage() {
  ...
  async function fetchProfissoes(pagina: number) {
    setLoading(true);
    try {
      const response = await api.get(`/profissao?page=${pagina}&limit=10`);
